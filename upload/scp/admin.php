...
if(defined('THIS_VERSION') && strcasecmp($cfg->getVersion(),THIS_VERSION)) {
    ...
    if(!$sysnotice && ini_get('register_globals'))
        $sysnotice='Por favor, considere desligar o registro global se possível.';
}
        ...
        case 'attach':
            if($_POST['allow_attachments'] or $_POST['upload_dir']) {
                ...
                if(!$_POST['allowed_filetypes'])
                    $errors['allowed_filetypes']='Extensão de arquivos permitidos necessária';
            }
            if(!$errors) {
        ...
        switch($do){
            ...
            case 'mass_process':
                if(!$_POST['tids'] || !is_array($_POST['tids'])) {
                    $errors['err']='Você deve selecionar pelo menos um t�pico.';
                }else{
                    ...
                    if($_POST['enable']){
                        ...
                        if(db_query($sql) && ($num=db_affected_rows()))
                            ...
                            $errors['err']='Não é possível desativar o(s) tópico(s) selecionado(s).';
                    }elseif($_POST['delete']){
                ...
                if(!$_POST['ids'] || !is_array($_POST['ids'])) {
                    ...
                    if($_POST['public']){
                        ...
                        if(db_query($sql) && ($num=db_affected_rows())) {
                            ...
                            $errors['err']='Não é possível fazer os departamentos selecionados serem privados. Possivelmente j� s�o privados!';
                        ...
                        if($members){
                            $errors['err']='Não pode excluir departamento com membros. Mova os atendentes primeiro.';
                             ...
                             $errors['err']='Não pode excluir departamento assiciados com tópicos de ajuda. Remova a associa��o primeiro.';
                        }else{
